...
-- Question_1: How many unique nodes are there on the Data Bank system?
select count(distinct node_id) as unique_nodes
from customer_nodes;

-- Question_2: What is the number of nodes per region?
  select r.region_id, r.region_name, count(cn.node_id) as count_node
...
-- Question_3: How many customers are allocated to each region?
select r.region_id, r.region_name, count(distinct cn.customer_id) as customer_count
from customer_nodes as cn
join regions as r
on cn.region_id = r.region_id
group by r.region_id, r.region_name
...
-- Question_4: How many days on average are customers reallocated to a different node?
WITH node_days AS (
  SELECT 
    customer_id, 
    node_id,
...
-- Question_5: What is the median, 80th and 95th percentile for this same reallocation days metric for each region?


										# B. Customer Transactions
-- Question_1: What is the unique count and total amount for each transaction type?
select txn_type, count(customer_id) as unique_customer, sum(txn_amount) as total_amount
from customer_transactions
group by txn_type;

-- Question_2: What is the average total historical deposit counts and amounts for all customers?
with cte_deposite as (
select count(customer_id) as dep_count,
		sum(txn_amount) as sum_amount
from customer_transactions
where txn_type = 'deposit'
group by customer_id)
select round(avg(dep_count)) as avg_cust, round(avg(sum_amount)) as avg_amount
from cte_deposite;
...
from cte_month
where dep_count >1 and (pur_count = 1 or with_count = 1)
group by 1;

-- Question_4: What is the closing balance for each customer at the end of the month?

-- Question_5: What is the percentage of customers who increase their closing balance by more than 5%?
